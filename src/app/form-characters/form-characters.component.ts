...
export class FormCharactersComponent implements OnInit {
  ...
  constructor(
    private fmService: FormcharacterServiceService,
    private its: ItemServiceService
  ) {}

  ngOnInit() {
    this.its.getAllItems().subscribe(itemList => {
  ...
  newCharacter() {
    ...
    this.fmService
      ...
      .subscribe(() => console.log("character added"));
      alert("Your build has been added to the database !");
  }
}
